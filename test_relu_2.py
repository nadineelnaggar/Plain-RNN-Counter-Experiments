...
from Dyck1_Datasets import NextTokenPredictionValidationDataset, NextTokenPredictionDataset1000tokens, \
    NextTokenPredictionDataset2000tokens_zigzag
...
if os.path.exists(
        '/Users/nadineelnaggar/Google Drive/PhD/EXPT_LOGS/Dyck1_NextTokenPrediction/Minibatch_Training/VanillaReLURNN/1_batch_size/0.01_learning_rate/30_epochs/50_lr_scheduler_step/1.0_lr_scheduler_gamma/1_hidden_units/20_runs/shuffle_True/'):
    prefix = '/Users/nadineelnaggar/Google Drive/PhD/EXPT_LOGS/Dyck1_NextTokenPrediction/Minibatch_Training/VanillaReLURNN/1_batch_size/0.01_learning_rate/30_epochs/50_lr_scheduler_step/1.0_lr_scheduler_gamma/1_hidden_units/20_runs/shuffle_True/'
        ...
        '/content/drive/MyDrive/PhD/EXPT_LOGS/Dyck1_NextTokenPrediction/Minibatch_Training/VanillaReLURNN/1_batch_size/0.01_learning_rate/30_epochs/50_lr_scheduler_step/1.0_lr_scheduler_gamma/1_hidden_units/20_runs/shuffle_True/'):
...
excel_name = prefix + 'INDICATORS_handmade_models_test_DEVS_A_B_U.xlsx'
...
class VanillaReLURNN__(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, batch_size, output_size, output_activation='Sigmoid',
                 rnn_input_weight=[1, -1], rnn_hidden_weight=[1]):
...
validation_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=shuffle_dataset,
...
def test_model(model, loader, dataset):
    ...
    return accuracy, total_loss / len(ds), total_bce_loss / len(
        ds), correct_guesses, correct_guesses_length, incorrect_guesses, incorrect_guesses_length, incorrect_guesses_first_fail, avg_first_fail_point, max_depths_correct_guesses, timestep_depths_correct_guesses, max_depths_incorrect_guesses, timestep_depths_incorrect_guesses
...
def main():
    ...
    # devs_a = []
    devs_b = [0.004, -0.004, 0.008, -0.008, 0.012, -0.012, 0.016, -0.016, 0.02, -0.02, 0]
    devs_u = [0.0001, -0.0001, 0.0002, -0.0002, 0.0003, -0.0003, 0.0004, -0.0004, 0.0005, -0.0005, 0]
    models = []

    for i in range(len(devs_a)):
        for k in range(len(devs_b)):
            for j in range(len(devs_u)):
                model_a_weight = 1 + devs_a[i]
                model_b_weight = -1 + devs_b[k]
                model_u_weight = 1 + devs_u[j]
                model_ = VanillaReLURNN__(input_size=2, hidden_size=1, output_size=2, num_layers=1, batch_size=1,
                                          output_activation='Sigmoid', rnn_input_weight=[model_a_weight, model_b_weight],
                                          rnn_hidden_weight=[model_u_weight])
    ...
    for i in range(len(models)):
        ...
        print('*******************************************************************')
        print('model ', i)
        test_accuracy, test_loss, test_bce_loss, test_correct_guesses, test_correct_guesses_length, test_incorrect_guesses, test_incorrect_guesses_length, test_incorrect_guesses_first_fail, test_avg_first_fail_point, max_depth_correct, timestep_depth_correct, max_depth_incorrect, timestep_depth_incorrect = test_model(
